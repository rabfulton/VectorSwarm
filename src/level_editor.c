...
static int is_regular_boid_wave_kind(int kind) {
    return (kind == LEVEL_EDITOR_MARKER_BOID ||
            kind == LEVEL_EDITOR_MARKER_BOID_FISH ||
            kind == LEVEL_EDITOR_MARKER_BOID_FIREFLY ||
            kind == LEVEL_EDITOR_MARKER_BOID_BIRD);
...
static int boid_style_from_marker_value(float value) {
    ...
    /* Backward-compat: old editor data used this slot for turn-rate degrees. */
...
static float boid_speed_default_for_kind(int kind) {
    if (kind == LEVEL_EDITOR_MARKER_EEL_SWARM) {
        return 250.0f;
    }
    if (kind == LEVEL_EDITOR_MARKER_JELLY_SWARM) {
        return 240.0f;
    }
...
static int wave_kind_has_speed_param_b(int kind) {
    return (kind == LEVEL_EDITOR_MARKER_WAVE_KAMIKAZE || is_boid_wave_kind(kind));
}

static int wave_kind_has_accel_param_c(int kind) {
    return (kind == LEVEL_EDITOR_MARKER_WAVE_KAMIKAZE || is_boid_wave_kind(kind));
}

static float wave_default_count_for_kind(int kind) {
    if (kind == LEVEL_EDITOR_MARKER_BOSS) {
        return 1.0f;
    }
    if (kind == LEVEL_EDITOR_MARKER_WAVE_SINE) {
        return 10.0f;
    }
    if (kind == LEVEL_EDITOR_MARKER_WAVE_V) {
        return 11.0f;
    }
    if (kind == LEVEL_EDITOR_MARKER_WAVE_KAMIKAZE) {
        return 9.0f;
    }
    if (kind == LEVEL_EDITOR_MARKER_MANTA_WING) {
        return 1.0f;
    }
...
static float wave_default_param_b_for_kind(int kind) {
    if (kind == LEVEL_EDITOR_MARKER_BOSS) {
        return 1234.0f;
    }
    if (kind == LEVEL_EDITOR_MARKER_WAVE_SINE) {
    ...
    if (is_boid_wave_kind(kind)) {
        return boid_speed_default_for_kind(kind);
    }
    return 0.0f;
}

static float wave_default_param_c_for_kind(int kind) {
    if (kind == LEVEL_EDITOR_MARKER_BOSS) {
        return 1.0f;
    }
    if (kind == LEVEL_EDITOR_MARKER_WAVE_SINE) {
    ...
    if (is_boid_wave_kind(kind)) {
        return boid_accel_default_for_kind(kind);
    }
    return 0.0f;
}
...
static float wave_default_param_e_for_kind(int kind) {
    if (kind == LEVEL_EDITOR_MARKER_BOSS) {
        return 0.0f;
    }
    if (is_regular_boid_wave_kind(kind)) {
        return 1.0f;
    }
    return 0.0f;
}

static int cycle_boss_blueprint_id(int current, int step) {
    int id = current;
    if (!boss_is_valid_blueprint_id(id)) {
        id = 1;
    }
    if (step == 0) {
        return id;
...
static void remap_wave_marker_fields_for_kind(level_editor_marker* m, int old_kind, int new_kind) {
    const float old_a = m ? m->a : 0.0f;
    const float old_b = m ? m->b : 0.0f;
    const float old_c = m ? m->c : 0.0f;
    const float old_d = m ? m->d : 0.0f;
    const float old_e = m ? m->e : 0.0f;
    float old_speed = 0.0f;
    float old_accel = 0.0f;
    int old_has_speed = 0;
    int old_has_accel = 0;
    if (!m || old_kind == new_kind || !is_wave_kind(new_kind)) {
        return;
    }

    if (old_kind == LEVEL_EDITOR_MARKER_WAVE_SINE || old_kind == LEVEL_EDITOR_MARKER_WAVE_V) {
        old_speed = old_c;
        old_has_speed = (old_c > 0.0f) ? 1 : 0;
    } else if (wave_kind_has_speed_param_b(old_kind)) {
        old_speed = old_b;
        old_has_speed = (old_b > 0.0f) ? 1 : 0;
    }
    if (wave_kind_has_accel_param_c(old_kind)) {
        old_accel = old_c;
        old_has_accel = (old_c > 0.0f) ? 1 : 0;
    }

    m->a = (old_a > 0.0f) ? old_a : wave_default_count_for_kind(new_kind);

    if (new_kind == LEVEL_EDITOR_MARKER_WAVE_SINE || new_kind == LEVEL_EDITOR_MARKER_WAVE_V) {
        if (old_kind == LEVEL_EDITOR_MARKER_WAVE_SINE || old_kind == LEVEL_EDITOR_MARKER_WAVE_V) {
            m->b = old_b;
            m->c = (old_c > 0.0f) ? old_c : wave_default_param_c_for_kind(new_kind);
        } else {
            m->b = wave_default_param_b_for_kind(new_kind);
            m->c = old_has_speed ? old_speed : wave_default_param_c_for_kind(new_kind);
        }
        m->d = 0.0f;
        m->e = 0.0f;
        return;
    ...
    if (new_kind == LEVEL_EDITOR_MARKER_MANTA_WING) {
        m->b = (old_kind == LEVEL_EDITOR_MARKER_MANTA_WING)
            ? clampf((old_b > 0.0f) ? old_b : wave_default_param_b_for_kind(new_kind), 1.0f, 4.0f)
            : wave_default_param_b_for_kind(new_kind);
        m->c = (old_kind == LEVEL_EDITOR_MARKER_MANTA_WING)
            ? clampf((old_c >= 0.0f) ? old_c : wave_default_param_c_for_kind(new_kind), 0.0f, 12.0f)
            : wave_default_param_c_for_kind(new_kind);
        m->d = 0.0f;
        m->e = 0.0f;
        return;
    }

    if (new_kind == LEVEL_EDITOR_MARKER_BOSS) {
        m->a = (old_kind == LEVEL_EDITOR_MARKER_BOSS && old_a > 0.0f) ? old_a : wave_default_count_for_kind(new_kind);
        m->b = (old_kind == LEVEL_EDITOR_MARKER_BOSS && old_b > 0.0f) ? old_b : wave_default_param_b_for_kind(new_kind);
        m->c = (old_kind == LEVEL_EDITOR_MARKER_BOSS && old_c > 0.0f) ? old_c : wave_default_param_c_for_kind(new_kind);
        m->d = (old_kind == LEVEL_EDITOR_MARKER_BOSS) ? old_d : wave_default_param_d_for_kind(new_kind);
        m->e = (old_kind == LEVEL_EDITOR_MARKER_BOSS) ? old_e : wave_default_param_e_for_kind(new_kind);
        return;
    }

    if (new_kind == LEVEL_EDITOR_MARKER_WAVE_KAMIKAZE || is_boid_wave_kind(new_kind)) {
        m->b = old_has_speed ? old_speed : wave_default_param_b_for_kind(new_kind);
        m->c = old_has_accel ? old_accel : wave_default_param_c_for_kind(new_kind);
        m->b = fmaxf(1.0f, m->b);
        m->c = fmaxf(0.1f, m->c);
        ...
        if (new_kind == LEVEL_EDITOR_MARKER_JELLY_SWARM || new_kind == LEVEL_EDITOR_MARKER_EEL_SWARM) {
            if (old_kind == LEVEL_EDITOR_MARKER_JELLY_SWARM || old_kind == LEVEL_EDITOR_MARKER_EEL_SWARM) {
                m->d = clampf((old_d > 0.0f) ? old_d : wave_default_param_d_for_kind(new_kind), 0.20f, 4.00f);
            } else {
                m->d = wave_default_param_d_for_kind(new_kind);
            }
            m->e = 0.0f;
        } else {
            if (old_kind == LEVEL_EDITOR_MARKER_BOID ||
                ...
                m->d = wave_default_param_d_for_kind(new_kind);
                m->e = wave_default_param_e_for_kind(new_kind);
            }
        }
    }
}

static int push_default_wave_marker(
    level_editor_state* s,
    int kind,
    int track,
    int order,
    ...
    push_marker(
        ...
        x01,
        y01,
        wave_default_count_for_kind(kind),
        wave_default_param_b_for_kind(kind),
        wave_default_param_c_for_kind(kind),
        wave_default_param_d_for_kind(kind)
    );
    if (s->marker_count > 0) {
        s->markers[s->marker_count - 1].e = wave_default_param_e_for_kind(kind);
    }
    return 1;
}
...
static const char* background_style_name(int style) {
    switch (style) {
        case LEVELDEF_BACKGROUND_NONE: return "none";
        case LEVELDEF_BACKGROUND_NEBULA: return "nebula";
...
static const char* curated_kind_name(int kind) {
    ...
    if (kind == LEVEL_EDITOR_MARKER_BOSS) return "boss_controller";
    return "sine";
}
...
static const char* marker_kind_name(int kind) {
    ...
    if (kind == LEVEL_EDITOR_MARKER_MANTA_WING) return "manta_wing";
    if (kind == LEVEL_EDITOR_MARKER_EEL_SWARM) return "eel_swarm";
    if (kind == LEVEL_EDITOR_MARKER_BOSS) return "boss";
    return "marker";
}
...
static int build_level_serialized_text(
    ...
    if (lvl.wave_mode == LEVELDEF_WAVES_CURATED) {
        ...
        for (i = 0; i < wave_n && curated_n < (int)(sizeof(lvl.curated) / sizeof(lvl.curated[0])); ++i) {
            ...
            lvl.curated[curated_n].d = m->d;
            lvl.curated[curated_n].e = is_regular_boid_wave_kind(m->kind)
                ? boid_size_scale_from_marker_value(m->e)
                : m->e;
            ++curated_n;
        }
    ...
    if (lvl.background_style == LEVELDEF_BACKGROUND_UNDERWATER) {
        ...
        if (!appendf(out, out_cap, &used, "underwater.caustic_scale=%.3f\n", lvl.underwater_caustic_scale)) return 0;
        if (!appendf(out, out_cap, &used, "underwater.bubble_rate=%.3f\n", lvl.underwater_bubble_rate)) return 0;
        ...
        if (!appendf(out, out_cap, &used, "underwater.kelp_tint_r=%.3f\n", lvl.underwater_kelp_tint_r)) return 0;
    ...
    } else if (lvl.background_style == LEVELDEF_BACKGROUND_ICE) {
        if (!appendf(out, out_cap, &used, "ice.voronoi_scale=%.3f\n", lvl.ice_voronoi_scale)) return 0;
        if (!appendf(out, out_cap, &used, "ice.crack_width=%.3f\n", lvl.ice_crack_width)) return 0;
        if (!appendf(out, out_cap, &used, "ice.distort_amp=%.3f\n", lvl.ice_distort_amp)) return 0;
        if (!appendf(out, out_cap, &used, "ice.parallax=%.3f\n", lvl.ice_parallax)) return 0;
        if (!appendf(out, out_cap, &used, "ice.shimmer=%.3f\n", lvl.ice_shimmer)) return 0;
    ...
    if (!appendf(out, out_cap, &used, "powerup.double_shot_r=%.3f\n", lvl.powerup_double_shot_r)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.double_shot_g=%.3f\n", lvl.powerup_double_shot_g)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.double_shot_b=%.3f\n", lvl.powerup_double_shot_b)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.triple_shot_r=%.3f\n", lvl.powerup_triple_shot_r)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.triple_shot_g=%.3f\n", lvl.powerup_triple_shot_g)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.triple_shot_b=%.3f\n", lvl.powerup_triple_shot_b)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.vitality_r=%.3f\n", lvl.powerup_vitality_r)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.vitality_g=%.3f\n", lvl.powerup_vitality_g)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.vitality_b=%.3f\n", lvl.powerup_vitality_b)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.orbital_boost_r=%.3f\n", lvl.powerup_orbital_boost_r)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.orbital_boost_g=%.3f\n", lvl.powerup_orbital_boost_g)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.orbital_boost_b=%.3f\n", lvl.powerup_orbital_boost_b)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.magnet_r=%.3f\n", lvl.powerup_magnet_r)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.magnet_g=%.3f\n", lvl.powerup_magnet_g)) return 0;
    if (!appendf(out, out_cap, &used, "powerup.magnet_b=%.3f\n", lvl.powerup_magnet_b)) return 0;
    ...
    if (!appendf(out, out_cap, &used, "exit_requires_boss_defeated=%d\n", lvl.exit_requires_boss_defeated ? 1 : 0)) return 0;
    ...
    if (lvl.wave_mode == LEVELDEF_WAVES_CURATED) {
        for (i = 0; i < lvl.curated_count; ++i) {
            ...
            if (!appendf(out, out_cap, &used, "curated_enemy=%s,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
                curated_kind_name(ce->kind), ce->x01, ce->y01, ce->a, ce->b, ce->c, ce->d, ce->e)) return 0;
        }
    } else {
...
static int marker_property_count(const level_editor_state* s) {
    ...
    if (is_wave_kind(kind)) {
        ...
        int tuned_count = 0;
        if (kind == LEVEL_EDITOR_MARKER_BOSS) {
            base_count = ev_item ? 7 : 8; /* event: TYPE,ORDER,DELAY,BOSS_ID,SEED,DIFFICULTY,VARIANT | spatial adds GATES_EXIT */
        } else if (is_regular_boid_wave_kind(kind)) {
            base_count = ev_item ? 8 : 9; /* event: TYPE,ORDER,DELAY,COUNT,SPEED,ACCEL,STYLE,SIZE | spatial adds DELAY */
        } else if (is_boid_wave_kind(kind)) {
            base_count = ev_item ? 7 : 8; /* event: TYPE,ORDER,DELAY,COUNT,SPEED,ACCEL,STYLE_OR_SIZE | spatial adds DELAY */
...
void level_editor_compute_layout(float w, float h, level_editor_layout* out) {
    ...
    out->exit_button = (vg_rect){
        out->entities.x + 8.0f * ui,
        out->entities.y + out->entities.h - 470.0f * ui,
        out->entities.w - 16.0f * ui,
        42.0f * ui
    };
    {
        ...
        for (int i = 0; i < lvl->curated_count; ++i) {
            ...
            push_marker(
                ...
                (ce->kind == LEVEL_EDITOR_MARKER_WAVE_KAMIKAZE)
                    ...
                    : (ce->kind == LEVEL_EDITOR_MARKER_BOSS)
                        ? ce->d
                    : (is_boid_wave_kind(ce->kind)
                        ? ((ce->kind == LEVEL_EDITOR_MARKER_JELLY_SWARM || ce->kind == LEVEL_EDITOR_MARKER_EEL_SWARM)
                            ? ((ce->d > 0.0f) ? ce->d : 1.0f)
                            : (float)boid_style_from_marker_value(ce->d))
                        : 0.0f)
            );
            if (s->marker_count > before) {
                if (is_regular_boid_wave_kind(ce->kind)) {
                    s->markers[before].e = boid_size_scale_from_marker_value(ce->e);
                } else if (ce->kind == LEVEL_EDITOR_MARKER_BOSS) {
                    s->markers[before].e = ce->e;
                }
            }
        }
        ...
        for (int i = 0; i < lvl->event_count; ++i) {
            ...
            cursor += fmaxf(ev->delay_s, 0.0f);
            {
                ...
                if (ev->kind == LEVELDEF_EVENT_ASTEROID_STORM) {
                    ...
                    push_marker(s, LEVEL_EDITOR_MARKER_BOID, LEVEL_EDITOR_TRACK_EVENT, ev->order, ev->delay_s, x01, p ? p->spawn_y01 : 0.5f, p ? p->count : 12.0f, p ? p->max_speed : boid_speed_default_for_kind(LEVEL_EDITOR_MARKER_BOID), p ? p->accel : boid_accel_default_for_kind(LEVEL_EDITOR_MARKER_BOID), (float)BOID_STYLE_CLASSIC);
                    ...
                    push_marker(s, LEVEL_EDITOR_MARKER_BOID_FISH, LEVEL_EDITOR_TRACK_EVENT, ev->order, ev->delay_s, x01, p ? p->spawn_y01 : 0.5f, p ? p->count : 12.0f, p ? p->max_speed : boid_speed_default_for_kind(LEVEL_EDITOR_MARKER_BOID_FISH), p ? p->accel : boid_accel_default_for_kind(LEVEL_EDITOR_MARKER_BOID_FISH), (float)BOID_STYLE_CLASSIC);
                } else if (ev->kind == LEVELDEF_EVENT_WAVE_SWARM_FIREFLY) {
                    const int pid = leveldef_find_boid_profile(db, "FIREFLY");
                    const leveldef_boid_profile* p = leveldef_get_boid_profile(db, (pid >= 0) ? pid : lvl->default_boid_profile);
                    push_marker(s, LEVEL_EDITOR_MARKER_BOID_FIREFLY, LEVEL_EDITOR_TRACK_EVENT, ev->order, ev->delay_s, x01, p ? p->spawn_y01 : 0.5f, p ? p->count : 12.0f, p ? p->max_speed : boid_speed_default_for_kind(LEVEL_EDITOR_MARKER_BOID_FIREFLY), p ? p->accel : boid_accel_default_for_kind(LEVEL_EDITOR_MARKER_BOID_FIREFLY), (float)BOID_STYLE_CLASSIC);
                } else if (ev->kind == LEVELDEF_EVENT_WAVE_SWARM_BIRD) {
                    const int pid = leveldef_find_boid_profile(db, "BIRD");
                    const leveldef_boid_profile* p = leveldef_get_boid_profile(db, (pid >= 0) ? pid : lvl->default_boid_profile);
                    push_marker(s, LEVEL_EDITOR_MARKER_BOID_BIRD, LEVEL_EDITOR_TRACK_EVENT, ev->order, ev->delay_s, x01, p ? p->spawn_y01 : 0.5f, p ? p->count : 12.0f, p ? p->max_speed : boid_speed_default_for_kind(LEVEL_EDITOR_MARKER_BOID_BIRD), p ? p->accel : boid_accel_default_for_kind(LEVEL_EDITOR_MARKER_BOID_BIRD), (float)BOID_STYLE_CLASSIC);
                } else {
...
static void add_marker_at_view(
    ...
    const int before_count = s ? s->marker_count : 0;
    int added = 0;
    if (!s || s->marker_count >= LEVEL_EDITOR_MAX_MARKERS) {
        return;
    }
    ...
    if (kind == LEVEL_EDITOR_MARKER_SEARCHLIGHT) {
        ...
        added = push_default_wave_marker(s, kind, LEVEL_EDITOR_TRACK_SPATIAL, 1, 0.0f, x01, y01);
...
static void add_spatial_marker_at_x01(level_editor_state* s, int kind, float x01, float y01) {
    ...
    if (kind == LEVEL_EDITOR_MARKER_SEARCHLIGHT) {
        ...
        added = push_default_wave_marker(s, kind, LEVEL_EDITOR_TRACK_SPATIAL, 1, 0.0f, xx, yy);
    } else if (kind == LEVEL_EDITOR_MARKER_ASTEROID_STORM) {
...
static void add_marker_at_timeline(level_editor_state* s, int kind, float x01) {
    ...
    const float cx = clampf(x01, 0.0f, 1.0f);
    {
        ...
        if (is_wave_kind(kind)) {
            const float y01 = (kind == LEVEL_EDITOR_MARKER_WAVE_V) ? 0.55f : 0.50f;
            added = push_default_wave_marker(s, kind, LEVEL_EDITOR_TRACK_EVENT, ord, 0.0f, cx, y01);
        } else if (kind == LEVEL_EDITOR_MARKER_ASTEROID_STORM) {
...
int level_editor_handle_mouse(level_editor_state* s, float mouse_x, float mouse_y, float w, float h, int mouse_down, int mouse_pressed) {
    ...
    if (mouse_pressed) {
        ...
        if (point_in_rect(mouse_x, mouse_y, l.boss_button)) {
            if (s->level_wave_mode != LEVELDEF_WAVES_CURATED) {
                s->level_wave_mode = LEVELDEF_WAVES_CURATED;
                mark_editor_dirty(s);
            }
            s->entity_tool_selected = LEVEL_EDITOR_MARKER_BOSS;
            s->entity_drag_active = 1;
            s->entity_drag_kind = LEVEL_EDITOR_MARKER_BOSS;
            s->entity_drag_x = mouse_x;
            s->entity_drag_y = mouse_y;
            return 1;
        }
        ...
        if (point_in_rect(mouse_x, mouse_y, l.viewport)) {
            ...
            } else if (((level_editor_enemy_spatial(s) && is_wave_kind(s->entity_tool_selected)) ||
                        is_boid_wave_kind(s->entity_tool_selected)) ||
                       s->entity_tool_selected == LEVEL_EDITOR_MARKER_SEARCHLIGHT ||
                       s->entity_tool_selected == LEVEL_EDITOR_MARKER_ASTEROID_STORM ||
                       s->entity_tool_selected == LEVEL_EDITOR_MARKER_MINEFIELD ||
                       s->entity_tool_selected == LEVEL_EDITOR_MARKER_MISSILE ||
...
int level_editor_handle_mouse_release(level_editor_state* s, float mouse_x, float mouse_y, float w, float h) {
    ...
    if (point_in_rect(mouse_x, mouse_y, l.viewport)) {
        ...
        if (s->entity_drag_kind == LEVEL_EDITOR_MARKER_SEARCHLIGHT ||
            ((level_editor_enemy_spatial(s) && is_wave_kind(s->entity_drag_kind)) ||
...
const char* level_editor_selected_property_name(const level_editor_state* s) {
    ...
    if (s->selected_marker >= 0 && s->selected_marker < s->marker_count) {
        const level_editor_marker* m = &s->markers[s->selected_marker];
        if (m->kind == LEVEL_EDITOR_MARKER_BOSS) {
            const int ev_item = marker_is_event_item(s, m);
            const int idx = (s->selected_property >= 0)
                ? clampi(s->selected_property, 0, ev_item ? 6 : 7)
                : 0;
...
void level_editor_adjust_selected_property(level_editor_state* s, float delta) {
    ...
    if (is_wave_kind(m->kind)) {
        ...
        if (m->kind == LEVEL_EDITOR_MARKER_BOSS) {
            switch (s->selected_property) {
                case 0:
                    break;
                case 1:
                    move_marker_x(s, s->selected_marker, pos_delta);
                    break;
                case 2:
                    m->y01 = clampf(m->y01 + pos_delta, 0.0f, 1.0f);
                    break;
                case 3:
                    m->a = (float)cycle_boss_blueprint_id((int)lroundf(m->a), (delta >= 0.0f) ? 1 : -1);
                    break;
                case 4:
                    m->b = (float)lroundf(m->b + ((delta >= 0.0f) ? 1.0f : -1.0f));
                    break;
                case 5:
                    m->c = clampf(m->c + delta * 0.05f, 0.25f, 4.0f);
                    break;
                case 6:
                    m->d = (float)lroundf(m->d + ((delta >= 0.0f) ? 1.0f : -1.0f));
                    break;
                case 7:
                    m->e = (m->e > 0.5f) ? 0.0f : 1.0f;
                    break;
                default:
                    break;
            }
        ...
        switch (s->selected_property) {
            case 0:
            {
                const int old_kind = m->kind;
                m->kind = cycle_wave_kind(old_kind, (delta >= 0.0f) ? 1 : -1);
                remap_wave_marker_fields_for_kind(m, old_kind, m->kind);
            }
                break;
            case 1:
                if (ev_item) m->order = (int)fmaxf(1.0f, (float)m->order + ((delta >= 0.0f) ? 1.0f : -1.0f));
            ...
            case 6:
                if (m->kind == LEVEL_EDITOR_MARKER_BOSS) {
                    ...
                    if (m->kind == LEVEL_EDITOR_MARKER_JELLY_SWARM || m->kind == LEVEL_EDITOR_MARKER_EEL_SWARM) {
                        m->d = clampf(m->d + delta * 0.05f, 0.20f, 4.00f);
                    } else {
                        const int dir = (delta >= 0.0f) ? 1 : -1;
                        const int n = BOID_STYLE_WRAITH - BOID_STYLE_CLASSIC + 1;
            ...
            case 7:
                ...
                } else if (is_regular_boid_wave_kind(m->kind)) {
                    const float size = boid_size_scale_from_marker_value(m->e);
                    m->e = boid_size_scale_from_marker_value(size + delta * 0.05f);
            ...
            case 8:
                ...
                } else if (is_regular_boid_wave_kind(m->kind) && !ev_item) {
                    m->delay_s = fmaxf(0.0f, m->delay_s + delta * 0.1f);
                }
                break;
            case 9:
