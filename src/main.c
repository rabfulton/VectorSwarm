...
typedef struct arc_beam_pc {
    ...
    float color_hot[4]; /* rgb */
    float seg[4];       /* x0,y0,x1,y1 in shader space */
...
enum acoustics_page_id {
    ...
    ACOUSTICS_PAGE_EFFECTS = 3,
    ACOUSTICS_PAGE_MIXTAPE = 4,
...
typedef struct app {
    ...
    atomic_uint pending_shield_tests;
    ...
    atomic_int lightning_pan_i16;
    atomic_uint shield_lfo_phase_u16;
    ...
    char mod_track_labels[MAX_MOD_TRACKS][MIXTAPE_LABEL_CAP];
    int mod_track_count;
    int mod_track_index;
    int mod_music_playing;
    float mod_music_volume_01;
    int acoustics_mixtape_selected;
    int acoustics_mixtape_focus; /* 0=library, 1=playlist */
    int acoustics_mixtape_playlist_selected;
    int acoustics_mixtape_prev_up;
    int acoustics_mixtape_prev_down;
    ...
    uint32_t lightning_test_frames_left;
    float shield_phase;
    float shield_fm_phase;
    float shield_env;
    float shield_lp;
    float aux_phase;
    float aux_fm_phase;
    ...
    float lightning_fm_phase;
    float lightning_env;
    float lightning_lp;
    float lightning_crackle_env;
    ...
    int acoustics_shield_slot_selected;
    int acoustics_aux_slot_selected;
    int acoustics_lightning_slot_selected;
    int acoustics_fx_slot_selected;
    ...
    uint8_t acoustics_shield_slot_defined[ACOUSTICS_SLOT_COUNT];
    uint8_t acoustics_aux_slot_defined[ACOUSTICS_SLOT_COUNT];
    uint8_t acoustics_lightning_slot_defined[ACOUSTICS_SLOT_COUNT];
    uint8_t acoustics_fx_slot_defined[ACOUSTICS_SLOT_COUNT];
    ...
    float acoustics_shield_slots[ACOUSTICS_SLOT_COUNT][8];
    float acoustics_aux_slots[ACOUSTICS_SLOT_COUNT][8];
    float acoustics_lightning_slots[ACOUSTICS_SLOT_COUNT][8];
    float acoustics_fx_slots[ACOUSTICS_SLOT_COUNT][8];
    ...
    float acoustics_equipment_value_01[ACOUST_EQUIP_SLIDER_COUNT];
    float acoustics_effects_value_01[ACOUST_FX_SLIDER_COUNT];
...
static void trigger_lightning_test(app* a) {
    ...
    atomic_fetch_add_explicit(&a->pending_lightning_tests, 1u, memory_order_acq_rel);
...
static void trigger_pickup_test(app* a) {
    ...
    (void)audio_spatial_enqueue(a, (uint8_t)GAME_AUDIO_EVENT_FX2, 0.0f, 1.0f);
}
...
static void sanitize_track_label(const char* in_name, char* out, size_t out_cap) {
    size_t w = 0u;
    int last_space = 1;
    if (!out || out_cap == 0u) {
        return;
    ...
    for (size_t i = 0; in_name[i] != '\0' && w + 1u < out_cap; ++i) {
        unsigned char c = (unsigned char)in_name[i];
        if (c >= 'a' && c <= 'z') {
    ...
    if (w == 0u) {
        snprintf(out, out_cap, "TRACK");
        return;
    }
    out[w] = '\0';
}

static void build_default_playlist_path(app* a) {
    if (!a) {
        return;
    }
...
static int save_mod_playlist(app* a) {
    ...
    const char* candidates[] = {"../data/playlist.cfg", "data/playlist.cfg"};
    FILE* f = NULL;
    for (int i = 0; i < (int)(sizeof(candidates) / sizeof(candidates[0])); ++i) {
        f = fopen(candidates[i], "wb");
        if (f) {
    ...
    for (int i = 0; i < a->mod_playlist_count; ++i) {
        ...
        fprintf(f, "track=%s\n", a->mod_track_paths[idx]);
    }
    fclose(f);
    return 1;
}

static void mixtape_playlist_add_internal(app* a, int track_idx, int persist) {
    if (!a || track_idx < 0 || track_idx >= a->mod_track_count) {
        return;
    }
    for (int i = 0; i < a->mod_playlist_count; ++i) {
        if (a->mod_playlist_indices[i] == track_idx) {
            a->acoustics_mixtape_playlist_selected = i;
            return;
        }
    }
    if (a->mod_playlist_count >= MAX_MOD_TRACKS) {
        return;
    }
...
static void mixtape_playlist_add(app* a, int track_idx) {
    mixtape_playlist_add_internal(a, track_idx, 1);
}

static void mixtape_playlist_remove_selected(app* a) {
    if (!a || a->mod_playlist_count <= 0) {
        return;
    }
    int sel = a->acoustics_mixtape_playlist_selected;
    if (sel < 0) {
        sel = 0;
    }
    if (sel >= a->mod_playlist_count) {
        sel = a->mod_playlist_count - 1;
    }
    for (int i = sel; i + 1 < a->mod_playlist_count; ++i) {
        a->mod_playlist_indices[i] = a->mod_playlist_indices[i + 1];
    }
    a->mod_playlist_count -= 1;
    if (a->mod_playlist_count < 0) {
        a->mod_playlist_count = 0;
    }
    if (a->acoustics_mixtape_playlist_selected >= a->mod_playlist_count) {
        a->acoustics_mixtape_playlist_selected = a->mod_playlist_count - 1;
    }
    ...
    for (int i = 0; i < (int)(sizeof(candidates) / sizeof(candidates[0])); ++i) {
        ...
        while (fgets(line, sizeof(line), f)) {
            ...
            if (strncmp(line, "track=", 6) == 0) {
                ...
                for (int t = 0; t < a->mod_track_count; ++t) {
                    if (strcmp(a->mod_track_paths[t], p) == 0) {
                        mixtape_playlist_add_internal(a, t, 0);
                        break;
                    }
                }
            }
...
static void discover_mod_tracks(app* a) {
    ...
    for (int r = 0; r < (int)(sizeof(roots) / sizeof(roots[0])); ++r) {
        ...
        while ((ent = readdir(d)) != NULL) {
            ...
            sanitize_track_label(ent->d_name, a->mod_track_labels[a->mod_track_count], sizeof(a->mod_track_labels[a->mod_track_count]));
            a->mod_track_count += 1;
        }
        closedir(d);
...
static int playlist_contains_track(const app* a, int track_idx) {
    if (!a || track_idx < 0) {
        return 0;
    }
    for (int i = 0; i < a->mod_playlist_count; ++i) {
...
static void mod_sync_gameplay_playlist(app* a) {
    if (!a || a->menu.current != APP_SCREEN_GAMEPLAY) {
        return;
    }
...
static void acoustics_equipment_defaults(app* a) {
    if (!a) {
        return;
    }
    acoustics_equipment_defaults_init(a->acoustics_equipment_value_01);
}

static void acoustics_effects_defaults(app* a) {
    if (!a) {
        return;
    }
    acoustics_effects_defaults_init(a->acoustics_effects_value_01);
}
...
static acoustics_slot_view acoustics_make_slot_view(app* a) {
    ...
    v.shield_slot_selected = &a->acoustics_shield_slot_selected;
    v.aux_slot_selected = &a->acoustics_aux_slot_selected;
    v.lightning_slot_selected = &a->acoustics_lightning_slot_selected;
    v.fx_slot_selected = &a->acoustics_fx_slot_selected;
    ...
    v.shield_slot_defined = a->acoustics_shield_slot_defined;
    v.aux_slot_defined = a->acoustics_aux_slot_defined;
    v.lightning_slot_defined = a->acoustics_lightning_slot_defined;
    v.fx_slot_defined = a->acoustics_fx_slot_defined;
    ...
    v.shield_slots = a->acoustics_shield_slots;
    v.aux_slots = a->acoustics_aux_slots;
    v.lightning_slots = a->acoustics_lightning_slots;
    v.fx_slots = a->acoustics_fx_slots;
    v.value_01 = a->acoustics_value_01;
    v.combat_value_01 = a->acoustics_combat_value_01;
    v.equipment_value_01 = a->acoustics_equipment_value_01;
    v.effects_value_01 = a->acoustics_effects_value_01;
    return v;
}
...
static void apply_acoustics(app* a) {
    ...
    acoustics_runtime_view v = {
        ...
        .equipment_value_01 = a->acoustics_equipment_value_01,
        .effects_value_01 = a->acoustics_effects_value_01,
        ...
        .shield_sound = &a->shield_sound,
        .aux_sound = &a->aux_sound,
        .lightning_sound = &a->lightning_sound,
        .fx_sound = &a->fx_sound
    };
...
static int handle_acoustics_ui_mouse(app* a, int mouse_x, int mouse_y, int set_value) {
    ...
    if (a->acoustics_page == ACOUSTICS_PAGE_COMBAT) {
        ...
        display_count = ACOUST_FX_SLIDER_COUNT;
        for (int i = 0; i < display_count; ++i) {
            display_values[i] = acoustics_effects_value_to_ui_display(i, a->acoustics_effects_value_01[i]);
        }
    } else if (a->acoustics_page == ACOUSTICS_PAGE_EQUIPMENT) {
        display_count = ACOUST_EQUIP_SLIDER_COUNT;
        for (int i = 0; i < display_count; ++i) {
            display_values[i] = acoustics_equipment_value_to_ui_display(i, a->acoustics_equipment_value_01[i]);
        }
    } else {
        for (int i = 0; i < display_count; ++i) {
    ...
    for (int p = 0; p < 2; ++p) {
            ...
            if (mx >= b.x && mx <= b.x + b.w && my >= b.y && my <= b.y + b.h) {
                if (set_value) {
                    if (a->acoustics_page == ACOUSTICS_PAGE_SYNTH) {
                        ...
                        acoustics_capture_current_to_selected_effects_slot_view(&sv, (p == 0) ? 1 : 0);
                    } else {
                        acoustics_capture_current_to_selected_equipment_slot_view(&sv, (p == 0) ? 1 : 0);
                    }
            ...
            if (mx >= b.x && mx <= b.x + b.w && my >= b.y && my <= b.y + b.h) {
                if (set_value) {
                    if (a->acoustics_page == ACOUSTICS_PAGE_SYNTH) {
                        ...
                        if (p == 0) {
                            a->acoustics_lightning_slot_selected = s;
                            acoustics_load_effects_slot_to_current_view(&sv, 1, s);
                            apply_acoustics(a);
                        } else {
                            a->acoustics_fx_slot_selected = s;
                            acoustics_load_effects_slot_to_current_view(&sv, 0, s);
                            apply_acoustics(a);
                        }
                    } else {
                        if (p == 0) {
                            a->acoustics_shield_slot_selected = s;
                            acoustics_load_equipment_slot_to_current_view(&sv, 1, s);
                            apply_acoustics(a);
                        } else {
                            a->acoustics_aux_slot_selected = s;
                            acoustics_load_equipment_slot_to_current_view(&sv, 0, s);
                            apply_acoustics(a);
                        }
                    }
                    (void)acoustics_save_slots_view(&sv, a->acoustics_slots_path);
        ...
        if (mx >= sx0 && mx <= sx1) {
            ...
            if (a->acoustics_page == ACOUSTICS_PAGE_COMBAT) {
                ...
                const int fxidx = (p == 0) ? row : (8 + row);
                if (fxidx >= 0 && fxidx < ACOUST_FX_SLIDER_COUNT) {
                    a->acoustics_effects_selected = fxidx;
                    if (set_value) {
                        const float t = clampf((mx - sx0) / l.slider_w[p], 0.0f, 1.0f);
                        a->acoustics_effects_value_01[fxidx] = t;
                        apply_acoustics(a);
                    }
                }
            } else if (a->acoustics_page == ACOUSTICS_PAGE_SYNTH) {
                ...
                const int eidx = (p == 0) ? row : (8 + row);
                if (eidx >= 0 && eidx < ACOUST_EQUIP_SLIDER_COUNT) {
                    a->acoustics_equipment_selected = eidx;
                    if (set_value) {
                        const float t = clampf((mx - sx0) / l.slider_w[p], 0.0f, 1.0f);
                        a->acoustics_equipment_value_01[eidx] = t;
                        apply_acoustics(a);
                    }
                }
            }
...
static void audio_callback(void* userdata, Uint8* stream, int len) {
    ...
    uint32_t shield_tests = atomic_exchange_explicit(&a->pending_shield_tests, 0u, memory_order_acq_rel);
    uint32_t aux_tests = atomic_exchange_explicit(&a->pending_aux_tests, 0u, memory_order_acq_rel);
    ...
    const int lightning_pan_i16 = atomic_load_explicit(&a->lightning_pan_i16, memory_order_acquire);
    const float lightning_gain_ctl = clampf((float)lightning_gain_u16 / 65535.0f, 0.0f, 1.0f);
    const float lightning_pan_ctl = clampf((float)lightning_pan_i16 / 32767.0f, -1.0f, 1.0f);
    const uint32_t shield_lfo_phase_u16 = atomic_load_explicit(&a->shield_lfo_phase_u16, memory_order_acquire);
    if (thruster_tests > 0) {
        a->thruster_test_frames_left = a->audio_have.freq / 3u;
    }
    if (shield_tests > 0) {
        a->shield_test_frames_left = a->audio_have.freq;
    }
    if (aux_tests > 0) {
        a->aux_test_frames_left = a->audio_have.freq / 2u;
    }
    if (lightning_tests > 0) {
        a->lightning_test_frames_left = a->audio_have.freq;
    }
    const int thruster_effective_gate = thrust_gate || (a->thruster_test_frames_left > 0u);
    const int shield_effective_gate = shield_gate || (a->shield_test_frames_left > 0u);
    const int aux_effective_gate = (a->aux_test_frames_left > 0u);
    const int lightning_effective_gate = lightning_gate || (a->lightning_test_frames_left > 0u);
    ...
    while (audio_spatial_dequeue_ring(
        ...
        audio_spawn_combat_voice(
            a->combat_voices, AUDIO_COMBAT_VOICE_COUNT, &a->audio_rng, &ev,
            &a->enemy_fire_sound, &a->explosion_sound, &a->aux_sound, &a->lightning_sound, &a->fx_sound
        );
    }
    ...
    while (remaining > 0) {
...
#endif
        {
            ...
            const float lfo_inc = 8.6f / sr;
            const float shield_attack_s = fmaxf(0.001f, a->shield_sound.attack_ms * 0.001f);
            const float shield_release_s = fmaxf(0.001f, a->shield_sound.decay_ms * 0.001f);
            const float shield_cutoff = fmaxf(80.0f, a->shield_sound.cutoff_hz);
            const float shield_alpha = clampf((6.2831853f * shield_cutoff) / sr, 0.0f, 1.0f);
            const float aux_attack_s = fmaxf(0.001f, a->aux_sound.attack_ms * 0.001f);
            const float aux_release_s = fmaxf(0.001f, a->aux_sound.decay_ms * 0.001f);
            const float aux_cutoff = fmaxf(80.0f, a->aux_sound.cutoff_hz);
            const float aux_alpha = clampf((6.2831853f * aux_cutoff) / sr, 0.0f, 1.0f);
            const float lightning_attack_s = fmaxf(0.001f, a->lightning_sound.attack_ms * 0.001f);
            const float lightning_release_s = fmaxf(0.001f, a->lightning_sound.decay_ms * 0.001f);
            const float lightning_cutoff = fmaxf(80.0f, a->lightning_sound.cutoff_hz);
            const float lightning_alpha = clampf((6.2831853f * lightning_cutoff) / sr, 0.0f, 1.0f);

            for (uint32_t i = 0; i < n; ++i) {
                const float lfo = 0.5f + 0.5f * sinf(lfo_phase0 + lfo_inc * (float)(off + i));

                if (shield_effective_gate) {
                    a->shield_env += (1.0f - a->shield_env) * (1.0f - expf(-1.0f / (sr * shield_attack_s)));
                } else {
                    a->shield_env *= expf(-1.0f / (sr * shield_release_s));
                }
                const float shield_base_hz = fmaxf(20.0f, a->shield_sound.pitch_hz);
                const float shield_fm_hz = fmaxf(0.1f, a->shield_sound.fm_rate_hz);
                a->shield_fm_phase += 6.2831853f * shield_fm_hz / sr;
                if (a->shield_fm_phase > 6.2831853f) a->shield_fm_phase -= 6.2831853f;
                const float shield_fm = sinf(a->shield_fm_phase) * a->shield_sound.fm_depth_hz;
                a->shield_phase += 6.2831853f * (shield_base_hz + shield_fm) / sr;
                if (a->shield_phase > 6.2831853f) a->shield_phase -= 6.2831853f;
                float shield_s = sinf(a->shield_phase);
                shield_s += (audio_rand01_from_state(&a->audio_rng) * 2.0f - 1.0f) * a->shield_sound.noise_mix;
                a->shield_lp += (shield_s - a->shield_lp) * shield_alpha;
                const float shield_amp = a->shield_sound.level * (0.65f + 0.35f * lfo) * a->shield_env;
                const float shield_out = a->shield_lp * shield_amp;

                if (aux_effective_gate) {
                    a->aux_env += (1.0f - a->aux_env) * (1.0f - expf(-1.0f / (sr * aux_attack_s)));
                } else {
                    a->aux_env *= expf(-1.0f / (sr * aux_release_s));
                }
                const float aux_base_hz = fmaxf(20.0f, a->aux_sound.pitch_hz);
                const float aux_fm_hz = fmaxf(0.1f, a->aux_sound.fm_rate_hz);
                a->aux_fm_phase += 6.2831853f * aux_fm_hz / sr;
                if (a->aux_fm_phase > 6.2831853f) a->aux_fm_phase -= 6.2831853f;
                const float aux_fm = sinf(a->aux_fm_phase) * a->aux_sound.fm_depth_hz;
                a->aux_phase += 6.2831853f * (aux_base_hz + aux_fm) / sr;
                if (a->aux_phase > 6.2831853f) a->aux_phase -= 6.2831853f;
                float aux_s = sinf(a->aux_phase);
                aux_s += (audio_rand01_from_state(&a->audio_rng) * 2.0f - 1.0f) * a->aux_sound.noise_mix;
                a->aux_lp += (aux_s - a->aux_lp) * aux_alpha;
                const float aux_out = a->aux_lp * a->aux_sound.level * a->aux_env;

                if (lightning_effective_gate) {
                    a->lightning_env += (1.0f - a->lightning_env) * (1.0f - expf(-1.0f / (sr * lightning_attack_s)));
                } else {
                    a->lightning_env *= expf(-1.0f / (sr * lightning_release_s));
                }
                const float lightning_base_hz = fmaxf(20.0f, a->lightning_sound.pitch_hz);
                const float lightning_fm_hz = fmaxf(0.1f, a->lightning_sound.fm_rate_hz);
                const float lightning_crackle = clampf(a->lightning_sound.noise_mix, 0.0f, 1.0f);
                a->lightning_fm_phase += 6.2831853f * lightning_fm_hz / sr;
                if (a->lightning_fm_phase > 6.2831853f) a->lightning_fm_phase -= 6.2831853f;
                const float lightning_fm = sinf(a->lightning_fm_phase) * a->lightning_sound.fm_depth_hz;
                a->lightning_phase += 6.2831853f * (lightning_base_hz + lightning_fm) / sr;
                if (a->lightning_phase > 6.2831853f) a->lightning_phase -= 6.2831853f;
                if (lightning_effective_gate) {
                    const float strike_rate_hz = 10.0f + lightning_crackle * 1600.0f;
                    if (audio_rand01_from_state(&a->audio_rng) < strike_rate_hz / sr) {
                        a->lightning_crackle_env = 1.0f;
                    }
                }
                a->lightning_crackle_env *= expf(-1.0f / (sr * 0.006f));
                const float crack = (audio_rand01_from_state(&a->audio_rng) * 2.0f - 1.0f) *
                                    a->lightning_crackle_env * (0.20f + 0.80f * lightning_crackle);
                const float lightning_tone = sinf(a->lightning_phase) +
                                             0.35f * sinf(a->lightning_phase * 2.7f + 0.6f * sinf(a->lightning_fm_phase));
                float lightning_s = lightning_tone * 0.72f + crack;
                a->lightning_lp += (lightning_s - a->lightning_lp) * lightning_alpha;
                const float lightning_gain = (a->lightning_test_frames_left > 0u) ? 1.0f : lightning_gain_ctl;
                const float lightning_pan = (a->lightning_test_frames_left > 0u) ? 0.0f : lightning_pan_ctl;
                const float lightning_out = a->lightning_lp * a->lightning_sound.level * a->lightning_env * lightning_gain;
                const float lightning_l = lightning_out * sqrtf(fmaxf(0.0f, 0.5f * (1.0f - lightning_pan)));
                const float lightning_r = lightning_out * sqrtf(fmaxf(0.0f, 0.5f * (1.0f + lightning_pan)));

                a->audio_mix_tmp_c[i] += shield_out + aux_out + lightning_l;
                a->audio_mix_tmp_d[i] += shield_out + aux_out + lightning_r;
            }
        }
        for (uint32_t i = 0; i < n; ++i) {
        ...
        if (a->shield_test_frames_left > 0u) {
            if (a->shield_test_frames_left > n) {
                a->shield_test_frames_left -= n;
            } else {
                a->shield_test_frames_left = 0u;
            }
        }
        if (a->aux_test_frames_left > 0u) {
            if (a->aux_test_frames_left > n) {
                a->aux_test_frames_left -= n;
            } else {
                a->aux_test_frames_left = 0u;
            }
        }
        if (a->lightning_test_frames_left > 0u) {
            if (a->lightning_test_frames_left > n) {
                a->lightning_test_frames_left -= n;
            } else {
                a->lightning_test_frames_left = 0u;
            }
        }
    }
}
...
static void init_teletype_audio(app* a) {
    ...
    if (a->audio_dev != 0) {
        ...
        a->aux_env = 0.0f;
        a->aux_lp = 0.0f;
        a->lightning_phase = 0.0f;
        a->lightning_fm_phase = 0.0f;
        a->lightning_env = 0.0f;
        a->lightning_lp = 0.0f;
        ...
        atomic_store_explicit(&a->pending_shield_tests, 0u, memory_order_release);
        ...
        atomic_store_explicit(&a->lightning_pan_i16, 0, memory_order_release);
        atomic_store_explicit(&a->shield_lfo_phase_u16, 0u, memory_order_release);
...
static void shipyard_open_link(app* a, int link_idx) {
    ...
    trigger_fire_test(a);
    if (targets[link_idx] == APP_SCREEN_ACOUSTICS) {
...
static void handle_mixtape_controls(app* a, const game_input* in) {
    if (!a || !in || a->menu.current != APP_SCREEN_ACOUSTICS || a->acoustics_page != ACOUSTICS_PAGE_MIXTAPE) {
        return;
    }
    const int up_pressed = in->up && !a->acoustics_mixtape_prev_up;
    const int down_pressed = in->down && !a->acoustics_mixtape_prev_down;
    const int left_pressed = in->left && !a->acoustics_mixtape_prev_left;
    const int right_pressed = in->right && !a->acoustics_mixtape_prev_right;
    const int fire_pressed = in->fire && !a->acoustics_mixtape_prev_fire;
    ...
    if (a->acoustics_mixtape_focus == 0) {
        if (a->mod_track_count > 0) {
            int next_idx = a->mod_track_index;
            if (up_pressed) {
                next_idx = (next_idx + a->mod_track_count - 1) % a->mod_track_count;
            } else if (down_pressed) {
                next_idx = (next_idx + 1) % a->mod_track_count;
            }
            if (next_idx != a->mod_track_index) {
                (void)mod_load_track(a, next_idx, 1);
            }
            if (fire_pressed) {
                mixtape_playlist_add(a, a->mod_track_index);
            }
        }
    } else {
        if (a->mod_playlist_count > 0) {
            if (up_pressed) {
                a->acoustics_mixtape_playlist_selected =
                    (a->acoustics_mixtape_playlist_selected + a->mod_playlist_count - 1) % a->mod_playlist_count;
            } else if (down_pressed) {
                a->acoustics_mixtape_playlist_selected =
                    (a->acoustics_mixtape_playlist_selected + 1) % a->mod_playlist_count;
            }
            if (a->acoustics_mixtape_playlist_selected >= 0 && a->acoustics_mixtape_playlist_selected < a->mod_playlist_count) {
                const int idx = a->mod_playlist_indices[a->acoustics_mixtape_playlist_selected];
                if (idx >= 0 && idx < a->mod_track_count && idx != a->mod_track_index) {
                    (void)mod_load_track(a, idx, 1);
                }
            }
    ...
    a->acoustics_mixtape_prev_up = in->up ? 1 : 0;
    a->acoustics_mixtape_prev_down = in->down ? 1 : 0;
    a->acoustics_mixtape_prev_left = in->left ? 1 : 0;
    a->acoustics_mixtape_prev_right = in->right ? 1 : 0;
    a->acoustics_mixtape_prev_fire = in->fire ? 1 : 0;
}
...
static void init_planetarium_assets(app* a) {
    ...
    {
        const char* tape_candidates[] = {
            "assets/images/tape.svg",
            "../assets/images/tape.svg",
            "../../assets/images/tape.svg"
        };
        for (size_t i = 0; i < sizeof(tape_candidates) / sizeof(tape_candidates[0]); ++i) {
            vg_svg_asset* asset = NULL;
            if (vg_svg_load_from_file(tape_candidates[i], &sp, &asset) == VG_OK && asset) {
                a->tape_svg_asset = asset;
                break;
            }
        }
    }
...
static void cleanup(app* a) {
    ...
    if (a->tape_svg_asset) {
        vg_svg_destroy(a->tape_svg_asset);
        a->tape_svg_asset = NULL;
    }
    for (int i = 0; i < 4; ++i) {
...
static int create_underwater_resources(app* a) {
    ...
    VkShaderModuleCreateInfo fs_forest_ci = {
        .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
    ...
    stages[1].module = fs_forest;
    gp.renderPass = a->scene_render_pass;
    gp.pMultisampleState = &ms;
    if (!check_vk(vkCreateGraphicsPipelines(a->device, VK_NULL_HANDLE, 1, &gp, NULL, &a->forest_pipeline), "vkCreateGraphicsPipelines(forest)")) {
        if (a->ice_pipeline) {
            vkDestroyPipeline(a->device, a->ice_pipeline, NULL);
            a->ice_pipeline = VK_NULL_HANDLE;
        }
...
static int create_arc_beam_resources(app* a) {
    ...
    if (!check_vk(vkCreatePipelineLayout(a->device, &pli, NULL, &a->arc_beam_layout), "vkCreatePipelineLayout(arc beam)")) {
        return 0;
    }

    VkShaderModuleCreateInfo vs_ci = {
        .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
        .codeSize = v_type_arc_beam_vert_spv_len,
        .pCode = (const uint32_t*)v_type_arc_beam_vert_spv
    };
    VkShaderModuleCreateInfo fs_ci = {
        .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
        .codeSize = v_type_arc_beam_frag_spv_len,
        .pCode = (const uint32_t*)v_type_arc_beam_frag_spv
    };
    VkShaderModule vs = VK_NULL_HANDLE;
    VkShaderModule fs = VK_NULL_HANDLE;
    if (!check_vk(vkCreateShaderModule(a->device, &vs_ci, NULL, &vs), "vkCreateShaderModule(arc beam vs)")) {
        return 0;
    }
    if (!check_vk(vkCreateShaderModule(a->device, &fs_ci, NULL, &fs), "vkCreateShaderModule(arc beam fs)")) {
        vkDestroyShaderModule(a->device, vs, NULL);
        return 0;
    }
    ...
    VkGraphicsPipelineCreateInfo gp = {
        ...
        .pColorBlendState = &cb,
        .pDynamicState = &ds,
        .layout = a->arc_beam_layout,
        .renderPass = a->scene_render_pass,
        .subpass = 0
    };
    if (!check_vk(vkCreateGraphicsPipelines(a->device, VK_NULL_HANDLE, 1, &gp, NULL, &a->arc_beam_pipeline), "vkCreateGraphicsPipelines(arc beam)")) {
        vkDestroyShaderModule(a->device, fs, NULL);
        vkDestroyShaderModule(a->device, vs, NULL);
        return 0;
    }
...
static void record_gpu_forest(app* a, VkCommandBuffer cmd, float t) {
    ...
    (void)t;
#else
    if (!a || !cmd || !a->forest_pipeline || !a->underwater_layout || !a->underwater_desc_set) {
        return;
    }
    const leveldef_level* lvl = game_current_leveldef(&a->game);
    if (!lvl || lvl->background_style != LEVELDEF_BACKGROUND_FOREST) {
    ...
    pc.p7[1] = clampf(lvl->forest_branch_wobble_speed, 0.0f, 6.0f);
    pc.p7[2] = clampf(lvl->forest_biolume_pulse_freq, 0.0f, 6.0f);
...
static int arc_pair_energized_gpu(const arc_node_runtime* node, float t) {
    ...
    const float period = fmaxf(node->period_s, 0.10f);
    const float on_s = clampf(node->on_s, 0.0f, period);
    if (on_s <= 0.0f) {
        return 0;
    }
    return (fmodf(t + node->phase_s, period) <= on_s) ? 1 : 0;
}
...
static void record_gpu_arc_beam(app* a, VkCommandBuffer cmd, float t) {
    ...
    float col_hot[3];
    if (palette_mode == 1) {
        col_dim[0] = 0.82f; col_dim[1] = 0.58f; col_dim[2] = 0.20f;
        col_hot[0] = 1.00f; col_hot[1] = 0.90f; col_hot[2] = 0.52f;
    } else if (palette_mode == 2) {
        col_dim[0] = 0.20f; col_dim[1] = 0.72f; col_dim[2] = 0.96f;
        col_hot[0] = 0.72f; col_hot[1] = 0.96f; col_hot[2] = 1.00f;
    } else {
        col_dim[0] = 0.04f; col_dim[1] = 0.56f; col_dim[2] = 0.14f;
    ...
    for (int i = 0; i + 1 < g->arc_node_count && i + 1 < MAX_ARC_NODES; i += 2) {
        ...
        vkCmdPushConstants(cmd, a->arc_beam_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(pc), &pc);
        vkCmdDraw(cmd, 3, 1, 0, 0);
    }
#endif
}
...
static int record_submit_present(app* a, uint32_t image_index, float t, float dt, float fps) {
    ...
    */
    ...
    render_metrics metrics = {
        ...
        .acoustics_effects_selected = a->acoustics_effects_selected,
        ...
        .level_editor_curated_formation_fire_prob_mul = a->level_editor.level_curated_combat.formation.fire_prob_mul,
        .level_editor_curated_formation_cooldown_mul = a->level_editor.level_curated_combat.formation.cooldown_mul,
        .level_editor_curated_formation_shot_count = a->level_editor.level_curated_combat.formation.shot_count,
        .level_editor_curated_formation_aim_error_mul = a->level_editor.level_curated_combat.formation.aim_error_mul,
        .level_editor_curated_formation_projectile_speed_mul = a->level_editor.level_curated_combat.formation.projectile_speed_mul,
        .level_editor_curated_formation_spread_mul = a->level_editor.level_curated_combat.formation.spread_mul,
        .level_editor_curated_swarm_fire_prob_mul = a->level_editor.level_curated_combat.swarm.fire_prob_mul,
        .level_editor_curated_swarm_spread_prob_mul = a->level_editor.level_curated_combat.swarm.spread_prob_mul,
        .level_editor_curated_swarm_cooldown_mul = a->level_editor.level_curated_combat.swarm.cooldown_mul,
        .level_editor_curated_swarm_shot_count = a->level_editor.level_curated_combat.swarm.shot_count,
        .level_editor_curated_swarm_aim_error_mul = a->level_editor.level_curated_combat.swarm.aim_error_mul,
        .level_editor_curated_swarm_projectile_speed_mul = a->level_editor.level_curated_combat.swarm.projectile_speed_mul,
        .level_editor_curated_swarm_spread_mul = a->level_editor.level_curated_combat.swarm.spread_mul,
        .level_editor_curated_kamikaze_fire_prob_mul = a->level_editor.level_curated_combat.kamikaze.fire_prob_mul,
        .level_editor_curated_kamikaze_speed_mul = a->level_editor.level_curated_combat.kamikaze.speed_mul,
        .level_editor_curated_kamikaze_accel_mul = a->level_editor.level_curated_combat.kamikaze.accel_mul,
        .level_editor_curated_manta_fire_prob_mul = a->level_editor.level_curated_combat.manta.fire_prob_mul,
        .level_editor_curated_manta_missile_count_bonus = a->level_editor.level_curated_combat.manta.missile_count_bonus,
        .level_editor_curated_manta_missile_cooldown_mul = a->level_editor.level_curated_combat.manta.missile_cooldown_mul,
        .level_editor_curated_manta_missile_charge_mul = a->level_editor.level_curated_combat.manta.missile_charge_mul,
        .level_editor_curated_eel_fire_prob_mul = a->level_editor.level_curated_combat.eel.fire_prob_mul,
        .level_editor_curated_eel_arc_fire_rate_mul = a->level_editor.level_curated_combat.eel.arc_fire_rate_mul,
        .level_editor_curated_eel_arc_duration_mul = a->level_editor.level_curated_combat.eel.arc_duration_mul,
        .level_editor_curated_eel_arc_range_mul = a->level_editor.level_curated_combat.eel.arc_range_mul,
        .level_editor_curated_eel_arc_damage_interval_mul = a->level_editor.level_curated_combat.eel.arc_damage_interval_mul,
    ...
    for (int i = 0; i < ACOUSTICS_SLOT_COUNT; ++i) {
        if (a->acoustics_page == ACOUSTICS_PAGE_COMBAT) {
            ...
            metrics.acoustics_fire_slot_defined[i] = a->acoustics_lightning_slot_defined[i] ? 1 : 0;
            metrics.acoustics_thr_slot_defined[i] = a->acoustics_fx_slot_defined[i] ? 1 : 0;
        } else if (a->acoustics_page == ACOUSTICS_PAGE_EQUIPMENT) {
            metrics.acoustics_fire_slot_defined[i] = a->acoustics_shield_slot_defined[i] ? 1 : 0;
            metrics.acoustics_thr_slot_defined[i] = a->acoustics_aux_slot_defined[i] ? 1 : 0;
        } else {
    ...
    if (a->acoustics_page == ACOUSTICS_PAGE_COMBAT) {
        ...
        metrics.acoustics_fire_slot_selected = a->acoustics_lightning_slot_selected;
        metrics.acoustics_thr_slot_selected = a->acoustics_fx_slot_selected;
    } else if (a->acoustics_page == ACOUSTICS_PAGE_EQUIPMENT) {
        metrics.acoustics_fire_slot_selected = a->acoustics_shield_slot_selected;
        metrics.acoustics_thr_slot_selected = a->acoustics_aux_slot_selected;
    } else {
    ...
    for (int i = 0; i < ACOUST_FX_SLIDER_COUNT; ++i) {
        metrics.acoustics_effects_value_01[i] = a->acoustics_effects_value_01[i];
        metrics.acoustics_effects_display[i] = acoustics_effects_value_to_ui_display(i, a->acoustics_effects_value_01[i]);
    }
    ...
    for (int i = 0; i < MIXTAPE_MAX_TRACKS; ++i) {
        ...
        if (i < a->mod_track_count) {
            snprintf(metrics.acoustics_mixtape_track_labels[i], MIXTAPE_LABEL_CAP, "%s", a->mod_track_labels[i]);
        } else {
            metrics.acoustics_mixtape_track_labels[i][0] = '\0';
        }
    }
    if (a->mod_track_count > 0 && a->mod_track_index >= 0 && a->mod_track_index < a->mod_track_count) {
        metrics.acoustics_mixtape_track_name = a->mod_track_labels[a->mod_track_index];
    } else {
...
int main(void) {
    ...
    a.acoustics_equipment_selected = 0;
    a.acoustics_effects_selected = 0;
    ...
    acoustics_equipment_defaults(&a);
    acoustics_effects_defaults(&a);
    {
        acoustics_slot_view sv = acoustics_make_slot_view(&a);
    ...
    a.acoustics_mixtape_focus = 0;
    a.acoustics_mixtape_playlist_selected = 0;
    a.acoustics_mixtape_prev_up = 0;
    a.acoustics_mixtape_prev_down = 0;
    ...
    if (a.audio_ready && a.mod_track_count > 0) {
        ...
        if (start_track < 0 || start_track >= a.mod_track_count) {
            start_track = 0;
        }
        (void)mod_load_track(&a, start_track, 1);
    }
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&ev)) {
            if (ev.type == SDL_QUIT) {
                ...
                if (menu_is_screen(&a.menu, APP_SCREEN_CONTROLS) && a.controls_rebinding_action >= 0) {
                ...
                } else if (menu_is_screen(&a.menu, APP_SCREEN_ACOUSTICS) && ev.key.keysym.sym == SDLK_s) {
                    {
                        ...
                        if (a.acoustics_page == ACOUSTICS_PAGE_SYNTH) {
                            ...
                            acoustics_capture_current_to_selected_effects_slot_view(&sv, 1);
                            acoustics_capture_current_to_selected_effects_slot_view(&sv, 0);
                        } else if (a.acoustics_page == ACOUSTICS_PAGE_EQUIPMENT) {
                ...
                } else if (menu_is_screen(&a.menu, APP_SCREEN_ACOUSTICS) && a.acoustics_page != ACOUSTICS_PAGE_MIXTAPE && ev.key.keysym.sym == SDLK_UP) {
                    if (a.acoustics_page == ACOUSTICS_PAGE_COMBAT) {
                        a.acoustics_combat_selected =
                            (a.acoustics_combat_selected + ACOUST_COMBAT_SLIDER_COUNT - 1) % ACOUST_COMBAT_SLIDER_COUNT;
                    } else if (a.acoustics_page == ACOUSTICS_PAGE_EFFECTS) {
                        a.acoustics_effects_selected =
                            (a.acoustics_effects_selected + ACOUST_FX_SLIDER_COUNT - 1) % ACOUST_FX_SLIDER_COUNT;
                    } else if (a.acoustics_page == ACOUSTICS_PAGE_EQUIPMENT) {
                ...
                } else if (menu_is_screen(&a.menu, APP_SCREEN_ACOUSTICS) && a.acoustics_page != ACOUSTICS_PAGE_MIXTAPE && ev.key.keysym.sym == SDLK_DOWN) {
                    if (a.acoustics_page == ACOUSTICS_PAGE_COMBAT) {
                        a.acoustics_combat_selected = (a.acoustics_combat_selected + 1) % ACOUST_COMBAT_SLIDER_COUNT;
                    } else if (a.acoustics_page == ACOUSTICS_PAGE_EFFECTS) {
                        a.acoustics_effects_selected = (a.acoustics_effects_selected + 1) % ACOUST_FX_SLIDER_COUNT;
                    } else if (a.acoustics_page == ACOUSTICS_PAGE_EQUIPMENT) {
                ...
                } else if (menu_is_screen(&a.menu, APP_SCREEN_ACOUSTICS) && a.acoustics_page != ACOUSTICS_PAGE_MIXTAPE && ev.key.keysym.sym == SDLK_LEFT) {
                    float* v = NULL;
                    if (a.acoustics_page == ACOUSTICS_PAGE_COMBAT) {
                        v = &a.acoustics_combat_value_01[a.acoustics_combat_selected];
                    } else if (a.acoustics_page == ACOUSTICS_PAGE_EFFECTS) {
                        v = &a.acoustics_effects_value_01[a.acoustics_effects_selected];
                    } else if (a.acoustics_page == ACOUSTICS_PAGE_EQUIPMENT) {
                    ...
                    *v = clampf(*v - 0.02f, 0.0f, 1.0f);
                    apply_acoustics(&a);
                } else if (menu_is_screen(&a.menu, APP_SCREEN_ACOUSTICS) && a.acoustics_page != ACOUSTICS_PAGE_MIXTAPE && ev.key.keysym.sym == SDLK_RIGHT) {
                    float* v = NULL;
                    if (a.acoustics_page == ACOUSTICS_PAGE_COMBAT) {
                        v = &a.acoustics_combat_value_01[a.acoustics_combat_selected];
                    } else if (a.acoustics_page == ACOUSTICS_PAGE_EFFECTS) {
        ...
        } else if (a.menu.current == APP_SCREEN_ACOUSTICS && a.acoustics_page == ACOUSTICS_PAGE_MIXTAPE) {
            handle_mixtape_controls(&a, &in);
        } else {
            a.shipyard_prev_up = 0;
            a.shipyard_prev_down = 0;
            a.shipyard_prev_left = 0;
        ...
        if (a.audio_ready) {
            ...
            {
                const uint32_t g16 = (uint32_t)clampf(a.game.lightning_audio_gain * 65535.0f, 0.0f, 65535.0f);
                atomic_store_explicit(&a.lightning_gain_u16, g16, memory_order_release);
                const int p16 = (int)clampf(a.game.lightning_audio_pan * 32767.0f, -32767.0f, 32767.0f);
                atomic_store_explicit(&a.lightning_pan_i16, p16, memory_order_release);
            }
            {
                const float t_s = (float)SDL_GetTicks() * 0.001f;
                const float phase01 = fmodf((t_s * 8.6f) / 6.2831853f, 1.0f);
                const uint32_t u16 = (uint32_t)clampf(phase01 * 65535.0f, 0.0f, 65535.0f);
                atomic_store_explicit(&a.shield_lfo_phase_u16, u16, memory_order_release);
            }
        }
        if (a.audio_ready) {
